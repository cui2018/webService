...
public class RedisUtil {
     ...
     * @return 成功返回value 失败返回null
     */
    public Object getSerializable(String key, int indexdb) {
        ...
        try {
            jedis = jedisPool.getResource();
            jedis.select(indexdb);
            value = SerializationUtils.deserialize(jedis.get(bytes));
        } catch (Exception e) {
            log.error(e.getMessage());
        } finally {
            returnResource(jedis);
     ...
     * @return 成功 返回OK 失败返回 0
     */
    public String setSerializable(String key, Object value, int indexdb) {
        byte[] bytes = key.getBytes();
        ...
        try {
            jedis = jedisPool.getResource();
            jedis.select(indexdb);
            return jedis.set(bytes, serialize);
        } catch (Exception e) {
            log.error(e.getMessage());
            return "0";
        } finally {
     ...
     *@Date  2019/5/31
     */
    public boolean setbit(String key, Long bitIndexd, String value){
        Jedis jedis = null;
        boolean res = false;
        try {
            jedis = jedisPool.getResource();
            res = jedis.setbit(key, bitIndexd, value);
        } catch (Exception e) {
            log.error(e.getMessage());
        } finally {
            returnResource(jedis);
